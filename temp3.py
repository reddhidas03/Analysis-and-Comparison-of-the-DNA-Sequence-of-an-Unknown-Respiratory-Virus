...
print("Compare with Covid(SARS_COV2): ",comp_2)
...
X = ['GC % of Flu(H1N1)', 'GC % of Covid(sers-cov2) ', 'GC % of unknown'] 
Y = [GC_of_ifz/ 100 * 100, GC_of_cov/100 * 100, GC_of_unk/100 * 100]
plt.figure(4)
plt.title('GC (%) Comparison')
plt.bar(X,Y,color=(0.6, 0.4, 0.3, 0.5))

A = ['Influenza(H1N1) & Unknown', 'SERS-COV2 & Unknown'] 
B = [i_u, c_u]
plt.figure(5)
plt.title('SimiLorities (%)')
...
table=pd.DataFrame(
 ...
 'Covid(sers-cov2)' : [G_of_cov,A_of_cov,T_of_ifz,C_of_cov,GC_of_cov,len(Amino_Acid_cov),len(functional_proteins_corona)], 
